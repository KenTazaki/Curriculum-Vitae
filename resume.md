...
- MR向けの医師リスト販売プロダクトの開発エンジニア
- 機能の要件調整・設計
- コーディングおよびコードレビュー
  - Ruby on Rails
  - Next.js
  - GitHub を利用したコード管理/レビュー
  - RSpec/Jest/CircleCi によるテスト
- 運用・問い合わせの対応

### 2021/1 ~ 2024/8: メドピア株式会社
...
### 2017/11 ~ 2020/12 : 株式会社 スタートアップテクノロジー
...
複数プロダクトでRuby on Rails を利用したサーバーサイド開発を中心に、Next.js を利用したフロントエンド開発も担当しています。
プロダクトの機能改善においては、要件調整から実装まで一貫して関与し、ユーザーや社内メンバーからの問い合わせ対応も行っています。
また 2025年2月よりエンジニアチームのマネージャーを務めており、上下関係に依存せず、メンバーが集中して効率よく働けるようなチーム作りを意識し、1on1での振り返りなどを実施しています。

#### 技術スタック

- Ruby 3.4.2 (2025/6 現在)
  - Ruby 3.1.4 からサービスに関わっており、その間のバージョンには対応してきました。
- Ruby on Rails 7.2.2 (2025/6 現在)
- MySQL8.0
...
- ElasticSearch
  - 医師の情報を学会および論文データから検索する際に利用しています
  - バージョン 7.10 を利用しています
- Sentry
  - エラー監視で利用しています。Slack 連携して即時対応できるような体制で運用しています。
- Datadog
  - パフォーマンス監視のため、定期的にパフォーマンスが悪化していないか確認するために利用しています。

#### 学会・論文情報管理ツール(社内向けツール)

MR向けに医師データを納品するために利用する、社内ビジネスメンバー向けデータ管理 Web アプリケーション。
Ruby on Rails によってサーバーサイドと管理画面を構築しています。

- 納品ファイルの出力機能や、学会データや論文データの取り込み機能などの既存機能改善の設計・開発を担当
  - ビジネスサイドと要件調整やスケジュール調整も行い、設計・開発を行っています。
  - また設計段階のレビューで DB schema 変更時など、安全に変更を反映できるように手順レビューを徹底しています。

- ビジネスメンバーからの要望対応に加え、エンジニア主導で技術的負債の改善提案も実施
  - システム内で権限管理が実際の業務に則していない形になっていたため、実際の役割の整理と実装の修正をすることで適切に個人情報が扱われる様に改修。
  - 既存実装で不要な問い合わせクエリが発行されていたため、レスポンスが遅くなっている箇所の改善活動

#### 医師情報検索ツール

医師の検索・ランキング機能（学会発表や論文著作の実績に基づく）を提供する顧客MRユーザー向け Web アプリケーション。
サーバーサイドをRuby on Railsで、フロントエンドを Next.js で構築しています。

- 検索履歴保存・ランキングお気に入り機能など、繰り返し利用を想定した機能の新規設計・開発
  - ビジネス要件の調整からフロント/サーバーサイドの開発まで一貫して担当

- 既存実装の見直し
  - 既存処理でSQLが過剰に発行されてしまっている箇所や、トランザクション内でSELECTが走り必要以上にロックが長くなっている箇所の修正
  - 責務の肥大化した PORO クラスの分割・再設計

- 今後のシステムの開発方針の打ち合わせ
  - ユーザー数増加時の社内コスト分析や、将来的に取得すべきデータの提案など、売り方を含めた改善提案も実施

#### 問い合わせ作業

- ビジネスメンバーから依頼される調査依頼への対応
  - 機能化されていないが必要なデータの一時出力対応
  - 出力データの検証・調査

#### マネージメント業務

- 自律的に動けるチームを目指し、マイクロマネジメントを避けた支援を実施しています
  - 朝会で日ごとの業務内容を共有してもらい、すべて指示するわけではなく、困っていそう・手の動きが鈍っていそうな時に声をかけられるようにしています。
- 定期的に1on1でフィードバックを行っています
  - タスクに区切りがついた際などは、実装内容含めて振り返りを行い、強みと課題を明確にするようにしています。
  - 私自身にもメンバーから「こう動いて欲しい」という要望がないかも聞いています。
- チーム内での勉強会を企画
  - PRの振り返り(良い実装や指摘コメントの振り返りなど)や、技術トピックの共有を行い、ナレッジ蓄積の場にしています。
